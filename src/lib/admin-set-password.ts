...
/**
 * Commissioner may set a temporary password only for a claimed player seat
...
export function checkSetMemberPassword(
  ...
  if (!membershipId) {
    return { ok: false, error: "Pick the person who needs a temporary password." };
  }
  ...
  if (!nicknamesMatch(input.member.nickname, confirmNickname)) {
    return {
      ok: false,
      error: `Type ${input.member.nickname} again to confirm.`,
    };
  }
  if (typeof password !== "string" || password.length < TEMP_PASSWORD_MIN) {
    return {
      ok: false,
      error: `Temporary password must be at least ${TEMP_PASSWORD_MIN} characters.`,
    };
  }
  if (password.length > TEMP_PASSWORD_MAX) {
    return {
      ok: false,
      error: `Temporary password must be ${TEMP_PASSWORD_MAX} characters or fewer.`,
    };
  }
  if (!password.trim()) {
    return { ok: false, error: "Temporary password cannot be only spaces." };
  }
  return {
    ok: true,
...
export async function setMemberTemporaryPassword(args: {
  ...
  | { ok: true; nickname: string; emailMasked: string }
    ...
    args.db.auditLog.create({
      data: {
        ...
        details: JSON.stringify({
          ...
          note: "Commissioner set a temporary password. The password itself is not stored here.",
        }),
      },
    }),
  ]);
