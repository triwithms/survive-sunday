...
| **Eliminated** | Out for the season — you can still follow the pool and cheer (or roast) from the sidelines. |</pre>
      ...
      <section id="9-live-scores-digests">
        ...
        <p className="text-[var(--text-muted)] mb-2">Use the week dropdown (and arrows) to check past weeks. Live games keep team names and scores, with a TV-style strip: <strong>down &amp; distance</strong> (for example <strong>1ST &amp; 10</strong>) and <strong>quarter | clock</strong> (for example <strong>4TH | 9:00</strong>) from ESPN. A <strong>🏈</strong> beside the abbreviation (and a gold bar) marks who has the ball; the yard line (for example <strong>MIN 42</strong>) shows when ESPN has it. If the clock is missing, ESPN’s short status is used (for example <strong>END 2ND</strong> / <strong>HALF</strong>). Finals show <strong>Final</strong>. Upcoming games show today’s kickoff time (ET). Small team logos sit beside the abbreviations. <strong>Tap a game</strong> for timeouts, scoring plays, drives, and leaders when ESPN has them.</p>
