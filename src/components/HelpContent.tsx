...
| **Eliminated** | Out for the season — you can still follow the pool and cheer (or roast) from the sidelines. |</pre>
      ...
      <section id="8-notifications">
        ...
        <p className="text-[var(--text-muted)] mb-2"><strong>Pick backup:</strong> Account → <strong>Pick backup</strong> (or Admin → Roster). Choose another pool member. If you still have <strong>no pick</strong> within <strong>30 minutes</strong> of that week’s lock (first kickoff) — or, in Week 1, that pick’s game kickoff — we copy their team. We never overwrite a pick you already submitted. JaJa is set to copy from Gams. A server job (not only opening the app) applies this.</p>
...
| Can’t change my pick | Weeks 2+ freeze at week lock. Week 1: you can still change until your pick’s kickoff, but only onto a game that has not started. |`}</pre>
      ...
      <section id="14-for-commissioners-admins">
        ...
        <ul className="list-disc pl-5 space-y-1 text-[var(--text-muted)] mb-2">
          ...
          <li className="text-[var(--text-primary)]">**Roster:** open **Admin → Roster** to see every nickname and real name, and fix either if it’s wrong. Each change is audit-logged. You can also set <strong>If no pick within 30 min of kickoff, copy from</strong> for a player (JaJa copies Gams by default).</li>
...
| **Pick backup** | Optional: copy another member’s pick if you still have none within 30 minutes of lock / that game’s kickoff. |
