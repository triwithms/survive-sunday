...
| **Eliminated** | Out for the season — you can still follow the pool and cheer (or roast) from the sidelines. |</pre>
      ...
      <section id="10-team-pages">
        ...
        <p className="text-[var(--text-muted)] mb-2">**Schedule** and **Pick** list games with kickoff (or LIVE / Final score) and an informational favourite when ESPN has a real line (for example <strong>BUF favoured by 4.5</strong>). A pick&apos;em (spread 0) shows as <strong>Even (pick&apos;em)</strong>. Those lists do <strong>not</strong> show TV channel (CBS / FOX / TSN), quarter / down-distance, or injury count chips such as <strong>3 Q</strong> or <strong>2 Out · 2 Q</strong>. Live clock and down-and-distance stay on <strong>Scores</strong>. Open a <strong>team page</strong> for the ESPN injury report. If ESPN has no line, the favourite field stays blank.</p>
        <p className="text-[var(--text-muted)] mb-2"><strong>Team pages</strong> start with the team&apos;s record, this week&apos;s game (kickoff, TV if we have it, and the favourite when ESPN has a line), head coach (name + ESPN / Wikipedia links), and style, then key NFL players, full roster, an ESPN injury report (Out / Doubtful / Questionable / IR / suspension), and news headlines. Tap a <strong>player name</strong> for college, depth role, and any ESPN injury note. This is ESPN&apos;s public list, not the official NFL club report.</p>
