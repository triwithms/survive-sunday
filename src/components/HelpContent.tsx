  ...
  return (
    <article className="prose-survive space-y-6 text-sm leading-relaxed max-w-[68ch]">
      <aside className="card-glass border border-gold-400/30 p-4 space-y-2" aria-label="Feature availability">
        <p className="text-[var(--text-primary)]"><strong>Wave 1 is live:</strong> picks, mulligan, lock and pick privacy, standings, scores and grading, League, Schedule, team research, prior-pick import, commissioner admin, PWA install, joining, sign-in that stays on your phone, forgot-password codes (email or text), cell-number collection, and <strong>Account → Notification preferences</strong>.</p>
        <p className="text-[var(--text-primary)]"><strong>Wave 2 — Coming soon:</strong> H2H boxing gloves, banter and mute, an in-app notification centre inbox, WhatsApp, weekly digests, and close-game alerts.</p>
      </aside>
      <section id="onboarding-first-run">
        ...
        <ul className="list-disc pl-5 space-y-1 text-[var(--text-muted)] mb-2">
          ...
          <li className="text-[var(--text-primary)]"><strong>Live scores and standings</strong> show how the pool is progressing. Add a <strong>cell number</strong> for SMS, then open <strong>Account → Notification preferences</strong> to choose which emails/texts you get. Wave 2 social features are marked below as coming soon.</li>
        </ul>
...
| **Eliminated** | Out for the season — you can still follow the pool and cheer (or roast) from the sidelines. |</pre>
      ...
      <section id="8-notifications">
        ...
        <p className="text-[var(--text-muted)] mb-2"><strong>Account → Notification preferences:</strong> phone-friendly on/off toggles for this pool. They save on your seat (per user / membership). Email and SMS for pool events honour these toggles. <strong>Forgot password</strong> codes always send — they are not a preference.</p>
        <pre className="card-glass p-3 text-xs overflow-x-auto whitespace-pre-wrap mb-2">| Type | Default |
|------|---------|
| Missing pick reminder | On |
| Pick confirmed / changed | On |
| Results graded | On |
| Mulligan / eliminated | On |
| Pool announcements | On |
| Live scores | Off |
| Injury notes | Off |</pre>
        <p className="text-[var(--text-muted)] mb-2"><strong>Cell for SMS:</strong> after sign-in you may be asked for a Canadian or US cell number. You can <strong>Skip for now</strong>; add or change it anytime from <strong>Account → Add cell number</strong>. Numbers are stored in E.164 and used for SMS only (WhatsApp is later).</p>
        <p className="text-[var(--text-muted)] mb-2"><strong>Coming later:</strong> an in-app notification inbox, weekly picks digests, WhatsApp messaging, and close-game alerts.</p>
        <p className="text-[var(--text-muted)] mb-2">---</p>
      </section>
      ...
      <section id="12-privacy-accounts">
        ...
        <ul className="list-disc pl-5 space-y-1 text-[var(--text-muted)] mb-2">
          ...
          <li className="text-[var(--text-primary)]">**Cell number** is optional but recommended for SMS (missing-pick reminders and password-reset texts). You can skip the soft prompt and add or edit it later from **Account**.</li>
          <li className="text-[var(--text-primary)]">**Notification preferences:** **Account → Notification preferences**. Live scores and injury notes start **off**. Forgot-password codes always send.</li>
...
| Missed SMS | Add or update your cell under **Account**. Confirm the number is correct and that the toggle is on under Notification preferences. |
...
| **Notification preferences** | **Account** sheet toggles for email/SMS (per seat). Live scores and injury notes start off. Forgot-password codes always send. |
