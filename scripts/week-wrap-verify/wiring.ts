...
assert.match(tone, /weekWrapAutoIntro/);
assert.doesNotMatch(tone, /Funny placeholder|Drama placeholder/);
assert.doesNotMatch(tone, /openai|anthropic/i);
const auto = readFileSync("src/lib/week-wrap-auto.ts", "utf8");
assert.doesNotMatch(auto, /fetch\(|openai|anthropic|process\.env/i, "auto copy makes no API call");
assert.match(handoff, /automatic from facts/i);
assert.match(help, /automatic/i);
