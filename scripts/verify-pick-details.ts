/**
 * My pick game Details › reuses the Scores sheet (no database).
 *
...
const aria = pickDetailsAria(matchup(), "Miami Dolphins");
assert.match(aria, /Miami Dolphins/);
assert.match(aria, /MIA at SF/);
console.log("PASS  details aria names the team and matchup");
...
assert.match(card, /onDetails=\{openDetails\}/);
...
assert.match(sideBtn, /PickGameDetailsButton/);
assert.match(sideBtn, /onDetails && detailsAria/);
assert.match(sideBtn, /aria-label=\{`Pick \$\{side\.name\}/);
assert.doesNotMatch(sideBtn, /href=\{`\/team\//);
console.log("PASS  Pick stays primary; Details is a separate control");
...
assert.match(detailsBtn, />\s*Details\s*</);
...
console.log("PASS  Details › matches Scores styling and is tappable");
...
console.log("PASS  Help mentions Details on My pick");
