...
| Settings / Account | `src/app/(app)/account/page.tsx` — hub. Notify `/account/notifications`. Pick backup `/account/mirror` (Off / ranked auto ~5 min). Report `/account/report`. Logic: `src/lib/pick-mirror.ts`. Jobs: ensure-week, `/api/cron/pick-backup`, scores sync. |
