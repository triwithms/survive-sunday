...
| Player notices | `src/lib/notify.ts` → `notify-dispatch.ts`. Gatekeeper `resolveChannels` / `planNotice` (`src/lib/notify-channels.ts`, `notify-plan.ts`). Master + per-type channel on `NotificationPreference` (`masterOn`, `channelsJson`: `email` \| `sms` \| `both` \| `off`). Account UI: `features/account/NotificationPrefsForm` + `NotifyMasterToggle` / `NotifyTypeList`. Admin: `RosterNotifyPref` (same model), `POST /api/admin/notify-test`. GAME SMS footer: `notify-sms-footer.ts`. Mode: `NOTIFY_MODE` + `NOTIFY_ALLOWLIST` (`notify-mode.ts`). Production ops: [HANDOFF §4 notification ops](HANDOFF.md#notification-ops-production). Additive columns + v1 defaults: `notify-type-schema.ts`. SECURITY (password reset / sign-in codes via `otp-notify.ts`) ignores prefs and `NOTIFY_MODE`. |
