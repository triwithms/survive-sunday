...
| Admin elim notify | `src/lib/elimination-admin-alert.ts` + `elimination-admin-copy.ts` — after grade / missed-pick / import newly eliminates someone, notify Administrators (same multi-admin union as password-reset). Goes through `dispatchNotice` (`admin_alert`) so each Administrator’s own `User.notifyPref` chooses Email / SMS / both / none. Dedupe via `NotificationSend` (channel + outcome). |
| Player notices | `src/lib/notify.ts` → `notify-dispatch.ts`. Gatekeeper `resolveChannels` / `planNotice` (`src/lib/notify-channels.ts`, `notify-plan.ts`). Pref on `User.notifyPref` (`email` \| `sms` \| `both` \| `none`). Account UI: `NotificationPrefsForm` + `NotifyPrefSelect`. Admin: `RosterNotifyPref`, `POST /api/admin/notify-test`. Mode: `NOTIFY_MODE` + `NOTIFY_ALLOWLIST` (`notify-mode.ts`). Production ops: [HANDOFF §4 notification ops](HANDOFF.md#notification-ops-production) (Resend domain, keep allowlist, Twilio). Column + defaults: `notify-pref-schema.ts` / `scripts/default-notify-prefs.ts`. SECURITY (password reset / sign-in codes via `otp-notify.ts`) ignores prefs and `NOTIFY_MODE`. |
