...
| Player notices | `src/lib/notify.ts` → `notify-dispatch.ts`. Gatekeeper `resolveChannels` / `planNotice` (`src/lib/notify-channels.ts`, `notify-plan.ts`). Pref on `User.notifyPref` (`email` \| `sms` \| `both` \| `none`). Account UI: `NotificationPrefsForm` + `NotifyPrefSelect`. Admin: `RosterNotifyPref`, `POST /api/admin/notify-test`. Mode: `NOTIFY_MODE` + `NOTIFY_ALLOWLIST` (`notify-mode.ts`). Column + defaults: `notify-pref-schema.ts` / `scripts/default-notify-prefs.ts`. SECURITY (password reset / sign-in codes via `otp-notify.ts`) ignores prefs and `NOTIFY_MODE`. |
