...
6. **Same email for commissioner and player is OK.** If you already saved `robertgama@gmail.com` as the commissioner login, pick **Gams** on Join and use that same email and the password you already sign in with (the Join password box is your **existing** Sign in password, not a new one). Or **Sign in first**, then Join — you’ll see **You’re signed in — claim with one tap**. One login then holds both roles: **Player** (Gams on the board) and **Administrator**. Do not invent a second email. If Join says the email already has an account, tap **I already have this login — Sign in to claim**.
