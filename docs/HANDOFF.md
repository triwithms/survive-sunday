...
**Snapshot (13 September 2026, late):** latest `main` is what friends see on [survive-sunday.vercel.app](https://survive-sunday.vercel.app). Live tonight:
...
- **Pick backup:** a member can copy another player’s pick if they still have none within **30 minutes** of that week’s lock (or, in Week 1, that pick’s kickoff). JaJa is set to copy from **Gams**. A 15-minute server job applies this — opening the app is not required.
...
| **Roster** | See each nickname + real name (including Go Giants, Pauli, JaJa). Edit either when wrong. Set **If no pick within 30 min, copy from** (JaJa → Gams). Audit-logged. |
...
| JaJa seat + pick backup | this PR | **JaJa (Jacquie Gama)** is on Join as claimable (`jaja@survivesunday.demo`), Week 1 **KC** with Gams. **Account → Pick backup** / Admin → Roster: copy another member’s pick if you still have none within 30 minutes of lock (Week 1: that game’s kickoff). Server cron every 15 minutes. |
