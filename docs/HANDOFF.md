...
- **Spreads and logos:** real ESPN favourites written **“BUF favoured by 4.5”**. Pick / Schedule / team-this-week now use the same page-refresh path before rendering, so one game shows one spread line across those screens. Team marks are **local-only** transparent helmets at `public/helmets/{abbr}.png` — no white plates, no ESPN CDN ([#119](https://github.com/triwithms/survive-sunday/pull/119), [#124](https://github.com/triwithms/survive-sunday/pull/124)). Never letter badges.
- **Player-facing times:** lock/kickoff/news/injury timestamps are formatted with one shared Eastern helper and rendered as **ET** across My pick, Selections, Leaderboard, Scores, Schedule, and team pages.
