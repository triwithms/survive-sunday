...
- **Sign in** (`/login`): email + password, or Google if keys are set. Optional **Email me a sign-in code** (same Resend / Twilio path as Forgot password; texts only if a cell is saved). This is **not** a code at every login. Copy on the page: Join once; stay signed in on this phone; use Forgot password only with that same email; don’t use Forgot password before Join.
- **Join** (`/join`): pick yourself from the **live roster** (nickname + real name), invite code **`SUNDAY26`**, then your own email and password (min 6 characters). That claims the existing seat so Week 1 picks stay. A **personal link** from Admin (`/join?seat=…` or `/join?who=cannoli-stuffer`) opens Join with that name already picked. If the seat already has a real email, the page says it is claimed and links to Sign in — not a broken form. Practice `@survivesunday.demo` seats (and leftover `@pending.survivesunday.local` placeholders) are claimable. **One user, more than one role** (merged [PR #19](https://github.com/triwithms/survive-sunday/pull/19)): there is **no special admin account**. The same email can be **Player + Administrator**. Use **Playing as …** / **Admin tools** to switch. Commissioner email can claim a player seat (Gams). People not on the list can still join as a new player.
- **Forgot password?** on the sign-in page: we email (or text) a 6-digit code → new password → signed back in. Only after that friend has Joined with that email. This is **not** a code at every login. **Codes do not send until Resend keys are on Vercel** (section 4). That is still the group-invite blocker.
...
| **Personal Join links** | Admin card + each Roster row. Copy a per-person `/join?seat=…` link (and a nickname `?who=` alias). Send that link only to that friend. Claimed seats say Sign in instead. |
...
| Simple password reset (code by email or SMS) | **Merged / shipping** ([PR #7](https://github.com/triwithms/survive-sunday/pull/7)). Sign in → **Forgot password?** → 6-digit code is on `main`. Optional **Email me a sign-in code** on `/login` (same keys; not every-login 2FA). Set `RESEND_API_KEY` + `RESEND_FROM_EMAIL` on Vercel or emails will not send. Optional Twilio for texts. |
...
| **Personal Join link** | Per-person URL from Admin / Roster (`/join?seat=…` or `/join?who=cannoli-stuffer`). Opens Join with that seat picked. Claimed seats → Sign in. |
...
| **OTP / sign-in code** | One-time 6-digit code. Forgot password **or** optional **Email me a sign-in code** on `/login`. Not a code at every login. Codes send only after Resend keys are on Vercel. |
