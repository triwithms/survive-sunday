...
- **Week wrap:** After a pool week’s last game is final, cron `/api/cron/week-wrap` (12:00 UTC, morning America/Toronto) sends type `weekWrap` on the next Toronto morning — including Monday and Tuesday night finales. A missed run can still send for 7 Toronto days. One send per user per pool week (`NotificationSend`). Default channel is email. SMS is short facts; Funny copy stays on email. Admin → System sets tone and blocks, previews, **Send now** (ignores the morning wait and a skip), or **Skip this week** (auto only). **Save defaults** is what the next automatic send uses. Template copy only — no live LLM. `NOTIFY_MODE` / allowlist unchanged. Not in this version: NFL division standings, a YouTube wrap link, per-player team highlights.
