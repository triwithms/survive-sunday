...
- **Notifications:** Account **notifyPref** is Email / SMS / both / none (`NotificationPrefsForm`). Admin **Send test to me**. Admin → System → **Missing picks** lists blanks, sends, or copies a group-chat list while the current week is open; the daily cron stays a 24-hour backup and each send re-checks `Pick` before notifying. Production `NOTIFY_MODE` is **`allowlist`** (`NOTIFY_ALLOWLIST` working) — **keep allowlist** until Robert says **`live`** ([#133](https://github.com/triwithms/survive-sunday/pull/133)). Password-reset codes are **not** gated by prefs. Ops notes (Resend From, Twilio, US Email, no Admin CC): **section 4**. The first-run prompt still asks friends to **add their cell for SMS reminders** (**Not now** is fine).
...
Friends add their cell for SMS reminders on the first-run prompt, or later from Account. Missing-pick reminder **emails** (and texts if Twilio is set) go out only when that friend left **Missing pick reminder** on. The same Resend keys as Forgot password send the emails. The daily Vercel cron is the quiet backup and still sends only in the 24 hours before lock. Admin → System → **Missing picks** can list who is still blank, send those reminders, or copy a group-chat list any time the current week is still open. Immediately before each reminder the app re-checks that no Pick exists for that seat and week, so a just-saved pick is not nagged.
...
- **Week wrap:** After a pool week’s last game is final, cron `/api/cron/week-wrap` sends type `weekWrap` at **noon America/Toronto the next calendar day** (16:00 UTC in EDT, 17:00 UTC in EST). A Tuesday 11:40 p.m. ET finale is eligible starting Wednesday noon; a Monday night finale is eligible starting Tuesday noon. A missed noon can still send for 7 Toronto days. One send per user per pool week (`NotificationSend`). Default channel is email. SMS is short facts; Funny copy stays on email. Admin → System sets tone and blocks, previews, **Send now** (ignores the noon wait and a skip), or **Skip this week** (auto only). **Save defaults** is what the next automatic send uses. Template copy only — no live LLM. Email uses the same quiet game-notice footer as other player mail (small preferences link to `/account/notifications`). SMS adds `Prefs:` plus that URL only when the whole text stays within 160 GSM-7 characters; otherwise the link stays on the email. At send time the app searches the official NFL channel for a title starting `Every Touchdown of Week N` (`YOUTUBE_API_KEY`); a miss does not block the send. There is no Admin paste box for the video. `NOTIFY_MODE` / allowlist unchanged. Not in this version: NFL division standings, per-player team highlights, extra tones.
...
After Sign in on a **phone browser**, a card asks: **Do you want to add NFL Pool to your Home Screen?** **Yes** / **No — don’t ask again** / **Not now**. Not now asks again next Sign in. No never auto-asks; Help → **Install on Home Screen** (top of Help) shows the Yes steps (Close / Done to leave). If they already open the **Home Screen icon**, we mark installed and do not nag. Installed stays installed in Safari on the same origin (shared links do not trap them). Desktop never shows.
...
| Add to Home Screen + stay logged in on phone; also mobile web + desktop | **Built.** After Sign in on a phone, Yes / No — don’t ask again / Not now. Yes = Install or iOS … → scroll → Share → Add to Home Screen (Close / Done). Not now = next Sign in. No = Help → Install on Home Screen. Installed / standalone does not auto-nudge in Safari. Cookie is ~**90 days**. |
